...
_ws.vm.addInitialPromptGroup();
  ...
  {
    _ws.vm.addInitialPrompt('ID3450_i_3');  /* Before I transfer you, please say the name of the product you're calling about. */
  }
  // ELSECONDITION: Else
  else if (true)
  {
    ...
    {
      _ws.vm.addInitialPrompt('ID3450_i_2');  /* What product are you calling about? */
    }
    // ELSECONDITION: Else
    else if (true)
    {
      _ws.vm.addInitialPrompt('ID3450_i_1');  /* And finally, what product are you calling about? */
    }
  }
_ws.vm.addNomatchPromptGroup('1', 'sequence', false);
  _ws.vm.addNomatchPrompt('Global_r_1');  /* I'm sorry. */
  _ws.vm.addNomatchPrompt('ID3450_r_1');  /* What's the name of the product? */
_ws.vm.addNomatchPromptGroup('2', 'sequence', false);
  _ws.vm.addNomatchPrompt('Global_r_2');  /* My mistake. */
  _ws.vm.addNomatchPrompt('ID3450_r_2');  /* Please say the name of the product once more. */
_ws.vm.addNoinputPromptGroup('1', 'sequence', false);
  _ws.vm.addNoinputPrompt('Global_r_1');  /* I'm sorry. */
  _ws.vm.addNoinputPrompt('ID3450_r_1');  /* What's the name of the product? */
_ws.vm.addNoinputPromptGroup('2', 'sequence', false);
  _ws.vm.addNoinputPrompt('Global_r_2');  /* My mistake. */
  _ws.vm.addNoinputPrompt('ID3450_r_2');  /* Please say the name of the product once more. */
_ws.vm.addDisconfirmPromptGroup('1', 'sequence', false);
  _ws.vm.addDisconfirmPrompt('Global_r_1');  /* I'm sorry. */
  _ws.vm.addDisconfirmPrompt('ID3450_r_1');  /* What's the name of the product? */
_ws.vm.addDisconfirmPromptGroup('2', 'sequence', false);
  _ws.vm.addDisconfirmPrompt('Global_r_2');  /* My mistake. */
  _ws.vm.addDisconfirmPrompt('ID3450_r_2');  /* Please say the name of the product once more. */
  ...
  }
{
  var promptArray = [];
  var promptCounter = 0;
  promptArray[promptCounter++] = [ 'ID3450_c_1', 'messageid' ];  /* Was that ninety one ten?  Yes or no? */
  // TODO addMenuItem: First parameter must match the grammar return data
  _ws.vm.addMenuItem('9110', 'CHOICE',  'null', promptArray);
}
{
  var promptArray = [];
  var promptCounter = 0;
...
}
{
  var promptArray = [];
  var promptCounter = 0;
  promptArray[promptCounter++] = [ 'ID3450_c_45', 'messageid' ];  /* Was that Accuchek?  Yes or no? */
  // TODO addMenuItem: First parameter must match the grammar return data
...
}
{
  var promptArray = [];
  var promptCounter = 0;
  promptArray[promptCounter++] = [ 'ID3450_c_47', 'messageid' ];  /* Was that Cobas?  Yes or no? */
  // TODO addMenuItem: First parameter must match the grammar return data
...
_ws.vm.setGrammar('ID3450_ITBloodGasProductName.grxml', 'voice');
...
Several confirmation prompts from this module are being used in module 3400.  If confirmation prompts are changed in this module or in that one, both tables need to evaluated to determine if both may safely change, or if each module needs its own prompts instead.*/
